    ...
    public function viewAction()
    {
        ...
        if ($request->isPost()) {
            ...
            if ($detail['REQUESTED_TYPE'] == 'ad') {
                ...
                $this->makeDecision1($id, $role, $action == 'Approve', $postedData[$role == 2 ? 'recommendedRemarks' : 'approvedRemarks'], true);
            }
            return $this->redirect()->toRoute("travelApprove");
        }
    ...
    private function makeDecision1($id, $role, $approve, $remarks = null, $enableFlashNotification = false)
    {
        ...
        switch ($role) {
            case 2:
                ...
                $model->recommendedBy = $this->employeeId;
                $model->status = $approve ? "RC" : "R";
                $message = $approve ? "Travel Expense Request Recommended" : "Travel Expense Request Rejected";
                $notificationEvent = $approve ? NotificationEvents::TRAVEL_EXPENSE_APPROVED : NotificationEvents::TRAVEL_EXPENSE_REJECTED;
                break;
            case 4:
                $model->recommendedDate = Helper::getcurrentExpressionDate();
                $model->recommendedBy = $this->employeeId;
            case 3:
                ...
                $model->approvedBy = $this->employeeId;
                $model->status = $approve ? "AP" : "R";
                $message = $approve ? "Travel Expense Request Approved" : "Travel Expense Request Rejected";
                $notificationEvent = $approve ? NotificationEvents::TRAVEL_EXPENSE_APPROVED : NotificationEvents::TRAVEL_EXPENSE_REJECTED;
                break;
        }
        $editError = $this->repository->edit($model, $id);
